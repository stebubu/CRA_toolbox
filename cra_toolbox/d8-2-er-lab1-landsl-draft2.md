...
The first phase of the workflow concerns the collection and f**ormatting of the core datasets required by CRITERIA-3D**.
...
Moreover, the .ini file text order has not to be modified for the same reason, in order to successfully build a project and run a simulation through CRITERIA-3D.

Users can only modify:

\- the parameters inside the databases, if a personal case study wants to be run.

\- the soil, land use and DEM maps, if a personal case study wants to be run.

\- the name of the specific database or map that CRITERIA-3D should use in its simulations, if one of the specific cases provided (BAU, SMART or REWILD) wants to be reproduced, through the .ini file. It is very important to indicate the precise file name inside the .ini file, otherwise CRITERIA-3D will not be able to follow the file path.

The three abovementioned cases represent three different Nature Based Solution (NBSs): respectively, coppicing (BAU), active conservative management (SMART) and free growing vegetation (REWILD). They are described more in detail in the Deliverable 1.2 of the ARCADIA project.
