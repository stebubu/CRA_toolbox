...
\*Scenario 1: Conversion of all croplands to extensive grassland (6.700 ha new wetlands)

\*\*Scenario 2. Conversion of all land uses except for urban land within the floodplain delineated by QSWAT+ to wetlands.
...
T**he flood hazard workflow** for the Vejrup Å catchment relies on a combination **of GIS platforms and flood modelling tools** (Table 4). GIS software provides the core environment for integrating elevation, land-use, and exposure datasets, producing flood maps and supporting spatial analysis.
...
High-resolution LiDAR-based DEM (0.4 m) from SDFI provides the elevation base for coastal flood simulations. Hydrological forcing is defined by **storm surge and sea level rise scenarios** following IPCC RCP 8.5 for 2022–2100, with return periods of 5, 10, 20, 50, and 100 years (Table 3). These scenarios set the boundary conditions for marine inundation.
...
The coastal flood model is configured using the prepared DEM, storm surge levels, and sea level rise increments. Using the flood modelling tool of choice (Table 4), storm surge return periods (e.g. 5, 10, 20, 50, 100 years) ,eventually worsened to consider sea level rise projections in climate change conditions (Table 3),are combined to delineate areas below the water thresholds (Figure 4)
